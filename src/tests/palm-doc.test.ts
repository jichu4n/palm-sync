...
import {ParseOptions} from 'src/serializable';
import PalmDoc, {PalmDocSerializeOptions} from '../palm-doc';
...
describe('PalmDoc', function () {
  ...
  for (const {label, text, serializeOpts, parseOpts} of [
    ...
    },
    {
      label: '日本語',
      text:
        '日本語（にほんご、にっぽんご、英: Japanese）は、主に日本国内や日本人同士の間で使用されている言語。',
      serializeOpts: {
        encoding: 'shiftjis',
        enableCompression: true,
      },
    ...
    test(`serialize ${label}`, async function () {
      ...
      doc2.parseFrom(buffer, parseOpts);
